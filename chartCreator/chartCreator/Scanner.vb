...
Public Module Scanner
    ...
    Public Sub checkImage()
        ...
        For r As Integer = 0 To chartRows - 1
            ...
            Dim rb As Integer = 0
            Dim y As Integer = 130 * r + 100                'Rows are 130px apart, plus 100px top margin
            For b As Integer = 1 To 32                      '32 possible beats in each row
                Dim x As Integer = 30 * b + 25              'Each small beat is 30px wide, plus 25px left margin
        ...
        mainForm.numBeats = nBeats                          'Remember the total number of beats
        If nBeats Mod 8 = 0 Then
            Dim nMeasures As Integer = CInt(nBeats / 8)
            If nMeasures <> guessMeasures Then
                ...
                If MessageBox.Show(userConfirm, "Timing", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                    'User confirmed number of measures is correct. Set each measure to 8 beats.
                    ReDim mainForm.beatMeasures(nMeasures)
                    For i As Integer = 0 To nMeasures - 1
                        mainForm.beatMeasures(i) = 8
                    Next
                ...
                End If
            Else
                'Guessed measures correctly. Set each measure to 8 beats.
                ReDim mainForm.beatMeasures(nMeasures)
                For i As Integer = 0 To nMeasures - 1
                    mainForm.beatMeasures(i) = 8
                Next
    ...
    Public Sub scanMeasures()
        ...
        For r As Integer = 1 To numRows
            ...
            For b As Integer = 1 To rowBound
                ...
                b = b - 1 + msrBeats
                Dim msrEnd As Integer = 30 * b + 24
                Dim msrWidth As Integer = msrBeats * 30
                ...
                'Check for BPM marker at each big beat in the measure
                Dim bigBeatsInThisMeasure = CInt(Math.Floor(msrBeats / 2))
                For beat As Integer = 1 To bigBeatsInThisMeasure
                    Dim beatStart As Integer = (msrStart - 60) + (beat * 60)
                    Dim yM As Integer = ry - 13
                    Dim bpmRect As New Rectangle(beatStart, yM, 4, 4)
                    ...
                    If cropColor = cBpm Then
                        ...
                        Dim bm As New bpmMarker
                        bm.Measure = msrNum
                        bm.Beat = beat * 2 - 1
                        bm.BPM = CDec(bm.getBpm(beatStart, ry))

                        ReDim Preserve mainForm.bpmMarkers(mainForm.bpmMarkers.GetUpperBound(0) + 1)
                        mainForm.bpmMarkers(mainForm.bpmMarkers.GetUpperBound(0) - 1) = bm
