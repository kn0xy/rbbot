...
void botStart()
{
	...
	while(notesReleased < noteCount) {
		if(bot_notesHit < noteCount) {
			...
			for(n=bot_notesHit; n < nub; n++) {
				if(chart_notes[n].ms - 10 <= bot_elapsedTime && chart_notes[n].hit == 0) {
					// Press button
					pressButton(chart_notes[n].color, 1);
					chart_notes[n].hit = 1;
					bot_notesHit++;
					if(chart_notes[n].color == 'G') {
						rg = bot_elapsedTime + chart_notes[n].duration + 10;
					} else if(chart_notes[n].color == 'R') {
						rr = bot_elapsedTime + chart_notes[n].duration + 10;
					} else if(chart_notes[n].color == 'Y') {
						ry = bot_elapsedTime + chart_notes[n].duration + 10;
					} else if(chart_notes[n].color == 'B') {
						rb = bot_elapsedTime + chart_notes[n].duration + 10;
					} else if(chart_notes[n].color == 'O') {
						ro = bot_elapsedTime + chart_notes[n].duration + 10;
					}
		...
		if(chart_notes[bot_notesHit+1].color != chart_notes[bot_notesHit].color) {
			if(rg > 0) {
	...
	printf("\n%sBot finished after %i min %i sec.\n\n%s", KGRN, ttMins, ttSecs, KNRM);
	appRunning = false;
}
